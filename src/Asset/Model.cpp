...
{
	size_t period = file_name.find_last_of('.');
	assert(period != std::string::npos);
	
	std::string_view file_type(&file_name[period]);
	if (file_type == ".glacier")
		load_glacier(file_name);
...
Model::Model(const std::string& file_name, float xz_size, float max_height, float u, float v)
{
...
Model::Model(uint32_t v_slices, uint32_t h_slices)
...
Model::Model(float xz_size, float u, float v)
...
void Model::load_glacier(const std::string& file_name)
{
