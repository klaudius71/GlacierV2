...
LevelScript::LevelScript()
	: Script("LevelScript"), frames(0), update(0.0f), physics(0.0f), draw(0.0f), three(0.0f), two(0.0f), frame_time(0.0f)
{
}
...
void LevelScript::OnUpdate(float dt)
{
	...
	Renderer2D::PrintText(font, 0.0f, (float)fb.GetSize().y - font.GetGlyphWithMaxHeight().size.y * 0.5f, Colors::White, "update: %.4fms", update);
	Renderer2D::PrintText(font, 200.0f, (float)fb.GetSize().y - font.GetGlyphWithMaxHeight().size.y * 0.5f, Colors::White, "physics: %.4fms", physics);
